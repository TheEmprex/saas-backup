...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('realTimeMessagingApp', realTimeMessagingApp);
    });
    ...
    function realTimeMessagingApp() {
        return {
            ...
            async init() {
                ...
                // Listen to Echo connection events if application emits them on window (optional)
            ...
            // Helper to get CSRF safely
            _csrfToken() {
                return document.querySelector('meta[name="csrf-token"]')?.content || '';
            },

            // Generic fetch helper that respects window.apiFetch if present
            async _apiFetch(path, opts = {}) {
                // If a custom apiFetch exists and is a function, prefer it
                if (typeof window.apiFetch === 'function') {
                    return window.apiFetch(path, opts);
                }
                // Otherwise fallback to fetch, auto-handle JSON and FormData bodies
                const fetchOpts = {
                ...
                // If body is plain object -> JSON
                if (opts.body && !(opts.body instanceof FormData)) {
                    fetchOpts.headers['Content-Type'] = fetchOpts.headers['Content-Type'] || 'application/json';
                    fetchOpts.body = JSON.stringify(opts.body);
                } else if (opts.body instanceof FormData) {
                    fetchOpts.body = opts.body;
                    // Let the browser set Content-Type for FormData (do not set)
                }

                const res = await fetch(path, fetchOpts);
                const text = await res.text();
                // try parse JSON, but return raw when not JSON
                try {
            ...
            async loadConversations() {
                try {
                    ...
                    const resp = await this._apiFetch('/messages/conversations', {
                        headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': this._csrfToken() }
                    });
                    const data = (resp && resp.data) || {};
                    // normalize: data may be { conversations: [...] } or an array
                    const rawList = data.conversations || data.data?.conversations || data.conversations || data || [];
                    this.conversations = (Array.isArray(rawList) ? rawList : [])
            ...
            async loadMessages(conversationId) {
                ...
                try {
                    if (this._abort.msgs) try { this._abort.msgs.abort(); } catch (_) {}
                    this._abort.msgs = new AbortController();
                    const signal = this._abort.msgs.signal;
                    const resp = await this._apiFetch(`/messages/conversations/${conversationId}/messages`, { signal, headers: { 'Accept': 'application/json' } });
                    const data = resp.data || {};
                    const list = (data.messages || data.data?.messages || data || []);
                    this.messages = (Array.isArray(list) ? list : []).map(msg => this.formatMessage(msg));
                    const perPage = (data.per_page || data.meta?.per_page || data.pagination?.per_page || 20);
                    const hasMore = (data.pagination?.has_more !== undefined) ? !!data.pagination.has_more : (this.messages.length >= perPage);
            ...
            // Send a message
            async sendMessage() {
                if ((!this.newMessage.trim() && (this.selectedFiles?.length || 0) === 0) || this.sendingMessage || !this.selectedConversation) {
                    return;
                }

                const content = this.newMessage.trim();
                const hasFiles = (this.selectedFiles && this.selectedFiles.length > 0);
                // clear input optimistically (we keep `content` for fallback)
                this.newMessage = '';
                ...
                try {
                    ...
                    // optimistic placeholder for text-only messages
                    if (!hasFiles && content) {
                    ...
                    if (hasFiles) {
                        ...
                        response = await this._apiFetch(`/messages/send`, { method: 'POST', body: form });
                    } else {
                        response = await this._apiFetch(`/messages/send`, {
                            method: 'POST',
                            body: {
                                conversation_id: this.selectedConversation.id,
                                content: content,
                    ...
                    const serverMsgRaw = data.message || data.data || data;
                    const serverMsg = this.formatMessage(serverMsgRaw || {});
                    ...
                    if (this.selectedConversation) {
                        this.selectedConversation.last_message = { content: content || serverMsg.content || '' };
            ...
            async searchUsers() {
                ...
                this._searchTimer = setTimeout(async () => {
                    ...
                    try {
                        const resp = await this._apiFetch(`/messages/search-users?q=${encodeURIComponent(q)}`, { signal, headers: { 'Accept': 'application/json' } });
                        const data = resp.data || {};
            ...
            async startConversation(user) {
                try {
                    const resp = await this._apiFetch('/messages/send', {
                        method: 'POST',
                        body: { recipient_id: user.id, content: 'Hi!' }
                    });
                    const data = resp.data || {};
            ...
            async selectConversation(conversation) {
                ...
                if (this._currentSubscribedConvId && (window.Echo || typeof Echo !== 'undefined')) {
                    try { (window.Echo || Echo).leave(`conversation.${this._currentSubscribedConvId}`); } catch (_) {}
                }
                this.selectedConversation = conversation;
                await this.loadMessages(conversation.id);
                this.scheduleMarkRead();

                // Mark as read on the conversation object locally
            ...
            // Initialize WebSocket connection (Echo preferred, fallback to polling)
            async initializeWebSocket() {
                try {
                    const echo = (typeof window !== 'undefined' && window.Echo) ? window.Echo : (typeof Echo !== 'undefined' ? Echo : null);

                    if (echo) {
                        // listen for user-level events
                        try {
                            // prefer chaining but keep safe guards for older Echo implementations
                            echo.private?.(`user.${this.currentUserId}`)
                                ?.listen?.('.message.sent', (e) => this.handleNewMessage(e))
                                ?.listen?.('.message.read', (e) => this.handleMessageRead(e));
                        } catch (err) {
            ...
            startPolling() {
                // store interval id so we could clear it later if desired
            ...
            subscribeToConversation(conversationId) {
                try {
                    ...
                    const chan = echo.private?.(`conversation.${conversationId}`) || echo.private(`conversation.${conversationId}`);
                    if (!chan) return;

                    chan.listen?.('.message.sent', (e) => {
                        // Only handle if sender is not me
                        const senderId = e?.message?.sender_id ?? e?.message?.sender?.id ?? e?.sender_id ?? e?.data?.sender_id;
                        if (senderId != this.currentUserId) this.handleNewMessage(e);
                    });

                    chan.listen?.('.user.typing', (e) => {
                    ...
                    // whisper support
                    if (typeof chan.listenForWhisper === 'function') {
                        chan.listenForWhisper('typing', (e) => {
                            const convId = conversationId;
            ...
            handleNewMessage(event) {
                const payload = event.message || event.data?.message || event.data || event;
            ...
            formatMessage(msg) {
                const reply = msg.reply_to || msg.replied_to || null;
            ...
            async loadFolders() {
                try {
                    const resp = await this._apiFetch('/messages/api/message-folders', { headers: { 'Accept': 'application/json' } });
                    const data = resp.data || {};
            ...
            async createFolder() {
                ...
                try {
                    const resp = await this._apiFetch('/api/message-folders', {
                        method: 'POST',
                        body: { name: name, color: this.newFolder.color }
                    });
                    await this.loadFolders();
                    this.showNotification('Folder created', 'success');
                    const created = resp.data?.data || resp.data || resp.data;
            ...
            async moveConversationToFolder(folderId, conversationId) {
                try {
                    await this._apiFetch(`/messages/api/message-folders/${folderId}/conversations`, {
                        method: 'POST',
            ...
            async loadOlderMessages() {
                ...
                try {
                    ...
                    console.warn('Failed loadOlderMessages', err);
                    this.messagesHasMore = false;
                } finally {
                    this.loadingMessages = false;
                }
            ...
            handleTyping() {
                ...
                try {
                    ...
                    if (echo && typeof echo.private === 'function') {
                        try {
                            echo.private(`conversation.${this.selectedConversation.id}`).whisper('typing', {
