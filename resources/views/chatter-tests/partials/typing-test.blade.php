<div class="bg-white shadow-lg rounded-lg overflow-hidden">
    <div class="p-6 md:p-8">
        <div class="flex items-center mb-6">
            <div class="w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg mr-4 
                {{ $test->language === 'en' ? 'bg-blue-500' : 'bg-red-500' }}">
                {{ strtoupper($test->language) }}
            </div>
            <div>
                <h1 class="text-3xl font-bold text-gray-900">{{ $test->title }}</h1>
                <p class="text-md text-gray-600">Typing Test</p>
            </div>
        </div>

        @if(isset($recentResult) && $recentResult)
        ...
        <form id="typing-test-form" action="{{ route('typing-tests.submit', $test->language) }}" method="POST" class="hidden">
            @csrf
...
document.addEventListener('DOMContentLoaded', function() {
    const testContent = "{!! addslashes($test->content) !!}";
