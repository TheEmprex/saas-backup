...
export const useMessageFoldersStore = defineStore('messageFolders', {
  ...
  actions: {
    async fetchFolders() {
      this.loading = true
      try {
        const response = await window.axios.get('/api/message-folders')
    ...
    async createFolder(folderData) {
      try {
        const response = await window.axios.post('/api/message-folders', folderData)
    ...
    async updateFolder(folderId, updates) {
      try {
        const response = await window.axios.put(`/api/message-folders/${folderId}`, updates)
    ...
    async deleteFolder(folderId) {
      try {
        await window.axios.delete(`/api/message-folders/${folderId}`)
    ...
    async addConversationToFolder(folderId, conversationId) {
      try {
        await window.axios.post(`/api/message-folders/${folderId}/conversations`, {
    ...
    async removeConversationFromFolder(folderId, conversationId) {
      try {
        await window.axios.delete(`/api/message-folders/${folderId}/conversations/${conversationId}`)
        await this.fetchFolders() // Refresh folders to get updated conversation lists
        this.error = null
      } catch (err) {
    ...
    async reorderFolders(folderIds) {
      try {
        await window.axios.post('/api/message-folders/reorder', { folders: folderIds })
