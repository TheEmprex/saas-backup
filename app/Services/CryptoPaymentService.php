...
class CryptoPaymentService
{
     ...
     */
    public function createPaymentRequest(SubscriptionPlan $plan, User $user, string $cryptocurrency = 'BTC')
    {
        try {
     ...
     */
    public function generatePaymentQR(string $address, float $amount, string $cryptocurrency)
    {
        $qrData = match($cryptocurrency) {
            'BTC' => "bitcoin:{$address}?amount={$amount}",
            'ETH', 'USDT', 'USDC' => "ethereum:{$address}?value={$amount}",
